...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, SUPPORT_GROUP, REQUESTS_CHANNEL, SUPPORT_CHAT, LOG_CHANNEL, STICKERS, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT
...
async def send_request(bot, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        if message.chat.id == SUPPORT_GROUP:
            ...
            sent_request = await bot.send_message(REQUESTS_CHANNEL, script.REQUEST_TXT.format(message.from_user.mention, message.from_user.id, request), reply_markup=InlineKeyboardMarkup(buttons))
